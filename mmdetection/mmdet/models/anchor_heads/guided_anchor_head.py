...
class GuidedAnchorHead(AnchorHead):
    ...
    def __init__(
        ...
        # super(GuidedAnchorHead, self).__init__(num_classes,in_channels,feat_channels,anchor_scales,
        #     anchor_ratios,anchor_strides,anchor_base_sizes,target_means,target_stds, loss_cls, loss_bbox)
    ...
    def get_sampled_approxs(self,
        ...
        Args:
            ...
            img_metas (list[dict]): Image meta info. (h,w)
        ...
        for img_id, img_meta in enumerate(img_metas):
            ...
            for i in range(num_levels):
                ...
                # position is true
                inside_flags = (
                    torch.stack(inside_flags_list, 0).sum(dim=0) > 0)  # (h*w)
    ...
    def get_guided_anchors_single(self,
        ...
        Args:
            square (tensor): Squares of a single level.  [h*w,4]
        ...
        loc_pred = loc_pred.sigmoid().detach() # [1, h, w]
        if use_loc_filter:
            loc_mask = loc_pred >= self.loc_filter_thr
        else:
            loc_mask = loc_pred >= 0.0
        mask = loc_mask.permute(1, 2, 0).expand(-1, -1, self.num_anchors)  # [h,w,num_anchors]
        mask = mask.contiguous().view(-1) # [h*w]
        # calculate guided anchors
        squares = squares[mask]
        anchor_deltas = shape_pred.permute(1, 2, 0).contiguous().view(
            -1, 2).detach()[mask]  # [h*w, 2]
        bbox_deltas = anchor_deltas.new_full(squares.size(), 0)  # [h*w, 4]
