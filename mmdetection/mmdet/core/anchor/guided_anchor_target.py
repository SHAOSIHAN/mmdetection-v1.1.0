...
def ga_loc_target(gt_bboxes_list,
    ...
    for img_id in range(img_per_gpu):
        ...
        min_anchor_size = scale.new_full(
            (1, ), float(anchor_scale * anchor_strides[0]))  # 32
    ...
    loc_avg_factor = sum(
        [t.size(0) * t.size(-1) * t.size(-2) for t in all_loc_targets]) / 200  # across all img.
...
def ga_shape_target(approx_list,
    ...
    num_level_squares = [squares.size(0) for squares in square_list[0]]  # [h*w]
